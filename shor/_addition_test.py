...
def test_exact_commands_for_small_circuit():
    ...
    assert backend.received_commands == [cmd for cmds in [
        (MultiNot & c).generate_commands(Qureg([y, x, a])),
        (X & c).generate_commands(x),
        (Swap & x).generate_commands((a, c)),
        (X & c).generate_commands(y),
        (Swap & x).generate_commands((a, c)),
        (X & a).generate_commands(x),
        (MultiNot & c).generate_commands(Qureg([a, x, y])),
    ] for cmd in cmds]
    ...
    for _ in range(10):
        ...
        fuzz_permutation_against_circuit(
            ...
            engine_list=[
                ...
                LimitedCapabilityEngine(allow_toffoli=True, allow_classes=[MultiNot.__class__, Swap.__class__])],
